  ...
  return (
    <div className="w-full" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
        className="p-4 group/file block rounded-lg cursor-pointer w-full relative overflow-hidden bg-neutral-800 border border-neutral-700">
        <input
          ref={fileInputRef}
          ...
          <div className="relative w-full mt-4 max-w-xl mx-auto">
            {files.length > 0 &&
              files.map((file, idx) => (
                <motion.div
                  ...
                  className={cn(
                    "relative overflow-hidden z-40 bg-neutral-900 border border-neutral-700 flex flex-col items-start justify-start md:h-24 p-4 mt-4 w-full mx-auto rounded-md",
                    "shadow-sm"
                  )}>
                  ...
                  <div
                    ...
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="px-1 py-0.5 rounded-md bg-neutral-800 text-neutral-300">
                      {file.type}
                    </motion.p>
            ...
            {!files.length && (
              <motion.div
                ...
                className={cn(
                  "relative group-hover/file:shadow-2xl z-40 bg-neutral-900 border border-neutral-700 flex items-center justify-center h-24 mt-4 w-full max-w-[6rem] mx-auto rounded-md",
            ...
            {!files.length && (
              <motion.div
                ...
                className="absolute opacity-0 border border-dashed border-purple-500 inset-0 z-30 bg-transparent flex items-center justify-center h-24 mt-4 w-full max-w-[6rem] mx-auto rounded-md"></motion.div>
            )}
          </div>
        </div>
      </motion.div>
          ...
          return (
            <div
              ...
              className={`w-10 h-10 flex shrink-0 rounded-[2px] ${
                index % 2 === 0
                  ? "bg-neutral-950"
                  : "bg-neutral-950 shadow-[0px_0px_1px_3px_rgba(255,255,255,0.1)_inset]"
              }`} />
          );
        }))}
    </div>
