  ...
  return (
    ...
    <div className="min-h-screen w-full flex items-center justify-center p-4 py-12 relative z-10">
      <div className="w-full max-w-4xl">
        {!showQuestions ? (
          <div className="bg-neutral-800 rounded-2xl p-6 shadow-lg shadow-black/50 max-h-[calc(100vh-3rem)] overflow-y-auto">
          ...
          {profileData && (
            <div className="mt-8 space-y-6">
              <div className="border-t border-neutral-700 pt-6">
                <h2 className="text-2xl font-bold text-white mb-4">{profileData.name}</h2>
                {profileData.headline && (
                  <p className="text-lg text-purple-400 mb-2">{profileData.headline}</p>
                )}
                {profileData.location && (
                  <p className="text-neutral-400 mb-4">{profileData.location}</p>
                )}
                {profileData.about && (
                  <div className="mb-4">
                  ...
                  <div className="space-y-4">
                    {profileData.experiences.map((exp, idx) => (
                      <div key={idx} className="bg-neutral-900 rounded-lg p-4">
                        <h4 className="text-white font-semibold">{exp.title || exp.position}</h4>
                        {exp.company && <p className="text-purple-400">{exp.company}</p>}
                        {exp.duration && <p className="text-neutral-400 text-sm">{exp.duration}</p>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="border-t border-neutral-700 pt-6">
                  ...
                  <div className="space-y-4">
                    {profileData.educations.map((edu, idx) => (
                      <div key={idx} className="bg-neutral-900 rounded-lg p-4">
                        <h4 className="text-white font-semibold">{edu.school || edu.institution}</h4>
                        {edu.degree && <p className="text-purple-400">{edu.degree}</p>}
                        {edu.year && <p className="text-neutral-400 text-sm">{edu.year}</p>}
                      </div>
                    ))}
                  </div>
                </div>
